...
def get_data(data_path, pairs_path):
    qa_pairs = []
    sentences_length = []
    with open(data_path, "r", encoding='utf8') as f:
        ...
        for line in f:
            ...
            if os.path.exists(pairs_path) == False or os.stat(pairs_path).st_size == 0:  # if the trimmed pairs are already saved
                for idx in range(len(sentences)-1):
                    inputLine = normalize_string(sentences[idx].strip())           # .strip() removes start/end whitespace
                    targetLine = normalize_string(sentences[idx+1].strip())
...
def get_test_data(data_path, lines_path, max_length):
    lines = []
    with open(data_path, "r") as f:
        ...
        for line in f:
            sentences = line.split("__eou__")
            sentences.pop()
            inputLine = normalize_string(sentences[0].strip())
